...
async def print_changelogs(event, ac_br, changelog):
    changelog_str = (
        f"**⌯︙قام مطورين السورس بتحديث هيرثون **\n⌯︙**التـغييرات\n** {changelog}"
    )
...
async def update(event, repo, ups_rem, ac_br):
    ...
    jasme = await event.edit(
        "** ⌯︙تم تحديث سورس هيرثون بنجاح انتظر قليلا سوف نخبرك بعد اعادة التشغيل !**"
    )
...
async def upstream(event):
    ...
    event = await edit_or_reply(event, "**⌯︙يـتـم البـحـث عـن تـحديثـات سـورس هيرثون انـتـظـر**")
    ...
    if changelog == "" and not force_update:
        await event.edit(
            "**⌯︙سورس هيرثون محدث الى اخر اصدار **\n"
            f"**قـنـاة سـورس هيرثون** : @Herthon"
        )
        return repo.__del__()
    if conf == "" and not force_update:
        ...
        return await event.respond(
            f"⌔ :  لتحديث سورس هيرثون ارسل : `.تحديث الان` "
        )
    ...
    if conf == "الان":
        await event.edit("** ⌯︙جار تحـديـث سـورس هيرثون انـتـظـر قـليـلا 🔨**")
