#    جميع الحقوق لمطوري سورس هيرثون حصريا لهم فقط
    ...
    if event.sender_id == bot.uid:
        await tgbot.send_message(
            ...
            buttons=[
                                     ...
                                     [Button.url("المطـور 🔗", "https://t.me/y2d_3"), Button.inline(
        ...
        await tgbot.send_message(
            ...
            link_preview=False,
            buttons=[
                [custom.Button.inline("تنـصيب هيرثون  🐍", data="deploy")],
                [Button.url("تحتاج مسـاعدة ❓", "https://t.me/y2d_3")],
            ],
        )
...
async def settings(event):
    if event.sender_id == bot.uid:
        ...
        await event.answer("انت لا تستطيع استخدام البوت احصل على بوتك من @Herthon", alert=True)




@tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"rozname"))) 
async def settings(event):  #    قـسـم  الزغرفـة هيرثون 
    if event.sender_id == bot.uid:
        await event.delete()
        await tgbot.send_message(event.chat_id,
...
async def users(event):
    await event.delete()
    rorza = "تستطيع اختراق اي شخص عبر كود تيرمكس في هيرثون يمكنك اختراق المستخدمين الذي تملك كود تيرمكس الخاص بهم \n\n ارسل  /rz للعرض الاوأمر"
    await tgbot.send_message(event.chat_id, rorza)
